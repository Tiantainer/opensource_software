...
###  13.`issue_comments.py`
- **功能**:分析 GitHub 仓库中的问题评论数据。
- **主要功能**：
  - 获取指定 GitHub 仓库中的所有问题评论数据。
  - 对问题评论数据进行分析，如评论频率、评论者详情以及情感分析等。
  - 将分析结果保存为 CSV 文件，供进一步审阅或报告使用。
- **依赖库**： `requests`, `pandas`, `collections`, `datetime`, `os`, `time`
  
###  14.`issue_comments_vis.py`
- **功能**： 可视化问题评论分析结果。
- **主要功能**：
  - 生成问题评论数据的可视化图表，如：
   - 按问题类型或标签分类的评论分布。
